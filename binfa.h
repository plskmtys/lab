...
/** @brief Bináris fa adatszerkezet
*/
...
void fn_bifa_megjelenit(SDL_Renderer *renderer, TTF_Font* font, fa *node, int x, int y, int depth, int radius) {
    ...
    if (node->l) {
        ...
        SDL_RenderDrawLine(renderer, x, y, x - 150/(depth+1), y + 100);
        fn_bifa_megjelenit(renderer, font, node->l, x - 150/(depth+1), y + 100, depth + 1, radius);
    }
    if (node->r) {
        SDL_SetRenderDrawColor(renderer, 255, 255, 255, SDL_ALPHA_OPAQUE); // white
        SDL_RenderDrawLine(renderer, x, y, x + 150/(depth+1), y + 100);
        fn_bifa_megjelenit(renderer, font, node->r, x + 150/(depth+1), y + 100, depth + 1, radius);
    }
...
void bifa_megjelenit(fa *node, int x, int y, int depth, int radius) {
    ...
    SDL_Window *window = SDL_CreateWindow("Tree Display", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 720, 720, SDL_WINDOW_SHOWN);
    SDL_Renderer *renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);

    fn_bifa_megjelenit(renderer, font, node, x, y, depth, radius);
